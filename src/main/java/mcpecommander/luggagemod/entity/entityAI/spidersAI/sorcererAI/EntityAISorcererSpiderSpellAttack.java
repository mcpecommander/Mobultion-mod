...
public class EntityAISorcererSpiderSpellAttack extends EntityAIBase{
    ...
    {
		if(this.attacker.ticksExisted % 4 == 0){this.attacker.playSound(SoundEvents.ENTITY_ILLAGER_PREPARE_MIRROR, .5F, .5F);}
	...
	public void summonSpiders(){
    	int numberOfspiders =  this.attacker.getRNG().nextInt(3) + (2 * (int)(this.world.getDifficultyForLocation(this.attacker.getPosition()).getAdditionalDifficulty()));
    	...
    	for(int i = 0; i < numberOfspiders; i++){
    		...
    		if(this.attacker.getAttackTarget() != null){
    			...
    			spider.setLocationAndAngles(this.attacker.posX + (x * 2) + (3 * this.attacker.getRNG().nextDouble()), this.attacker.posY, this.attacker.posZ + (z * 2) + (3 * this.attacker.getRNG().nextDouble()), 0.0F, 0.0F);
	...
	private ArrayList<EntityLiving> getSpidersList(int num, World world){
    	...
    	for(int i = 0; i < num; i++){
    		...
    		case 2: list.add(new EntityCaveSpider(world));
    		break;
    		
    		case 3: list.add(new EntitySpeedySpider(world));
    		break;
    		
    		case 4: list.add(new EntityMagmaSpider(world));
    		break;
    		
    		case 5: list.add(new EntityHypnoSpider(world));
    		break;
    		
    		case 6: list.add(new EntityWitherSpider(world));
    		break;
    		
    		default: list.add(new EntitySpider(world));
    		break;
    		}
    	}
    	return list;
