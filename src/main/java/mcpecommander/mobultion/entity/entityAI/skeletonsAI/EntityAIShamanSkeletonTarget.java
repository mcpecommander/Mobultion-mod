...
public class EntityAIShamanSkeletonTarget extends EntityAITarget{

	private Class[] targets;
	private EntityLivingBase mainTarget;
	World world;
    protected final EntityAIShamanSkeletonTarget.Sorter sorter;
	
	public EntityAIShamanSkeletonTarget(EntityCreature entity, boolean checkSight, Class... targets) {
		super(entity, checkSight);
		this.targets = targets;
		this.world = entity.world;
		this.sorter = new EntityAIShamanSkeletonTarget.Sorter(entity);
	}
			...
			if(mainList.isEmpty()){
				...
				for(int i = 0; i< mainList.size(); i++){
					if(mainList.get(i) != null && !mainList.get(i).isDead){
						if(mainList.get(i) instanceof EntitySkeletonRemains){
