...
public class KnifeForkHoldingLayer extends GeoLayerRenderer<HungryZombieEntity> {
    ...
    @Override
    public void render(PoseStack matrix, MultiBufferSource renderBuffer, int packedLight, HungryZombieEntity entity,
        ...
        renderRecursively(entity, model.topLevelBones.get(0), matrix, renderBuffer, packedLight, OverlayTexture.pack(0,
    ...
    private void renderRecursively(Entity entity, GeoBone bone, PoseStack stack, MultiBufferSource renderBuffer, int packedLightIn, int packedOverlayIn) {
        ...
        if(bone.getName().equals("LeftArm1")){
            ...
            stack.scale(1f, 1f, 1f);
            // Change mainHand to predefined Itemstack and TransformType to what transform you would want to use.
            Minecraft.getInstance().getItemRenderer().renderStatic(offHand, ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND,
                    packedLightIn, packedOverlayIn, stack, renderBuffer, entity.getId());
            stack.popPose();
        }
